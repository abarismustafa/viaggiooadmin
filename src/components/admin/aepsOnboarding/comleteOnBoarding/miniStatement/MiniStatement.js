...
function MiniStatement() {
    ...
    const Capturefingerherw = () => {
        if (!position.latitude || !position.longitude) {
            toastErrorMessage("Location data is not available. Please enable location and refresh the page.");
            return;
        }
        try {
    ...
    const isValidInput = () => {
        return (
            initialValue.mobilenumber.length === 10 &&
            initialValue.adhaarnumber.length === 12 &&
            showBanak?.iinno
        );
    };
